...
for i in range(graphSizeY):
    newrow = inpu[i]
    for j in range(4):
        newrow = [element % 9+1 for element in newrow]
        inpu[i] += newrow

graphSizeX = len(inpu[0])

for j in range(4):
    for i in range(j*graphSizeY, (j+1) * graphSizeY):
        newrow = [element % 9+1 for element in inpu[i]]
        inpu.append(newrow)
...
print(D[graphSizeY * graphSizeX - 1])
